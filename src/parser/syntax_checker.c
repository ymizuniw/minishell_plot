...
{
	if (token->prev && (token->prev->type == TK_WORD
			|| token->prev->type == TK_DOLLER))
	{
...
int	check_operator(t_token *token)
{
	if ((token->next && token->next->type != TK_WORD
			&& token->next->type != TK_RPAREN) || (token->prev
			&& token->prev->type != TK_WORD
			&& token->prev->type != TK_REDIRECT_IN
			&& token->prev->type != TK_REDIRECT_OUT
			&& token->prev->type != TK_HEREDOC && token->prev->type != TK_APPEND
			&& token->prev->type != TK_LPAREN))
		return (0);
	return (1);
}
...
int	syntax_check(t_token *token)
{
	...
	if (token_type == TK_REDIRECT_IN || token_type == TK_HEREDOC
		|| token_type == TK_REDIRECT_OUT || token_type == TK_APPEND)
